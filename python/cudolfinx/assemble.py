...
"""GPU-accelerated assembly operations."""
...
def create_petsc_cuda_vector(L: Form) -> PETSc.Vec:
  """Create PETSc Vec on GPU from a Form."""
...
class CUDAAssembler:
    ...
    def assemble_matrix(self,
        ...
        bcs: typing.Union[list[DirichletBC], CUDADirichletBC] | None = [],
        diagonal: float = 1.0,
        ...
        if zero:
            _cucpp.fem.zero_matrix_entries(
                self._ctx,
                self._cpp_object,
                mat._cpp_object
            )
    ...
    def assemble_matrix_block(self,
        ...
        bcs: list[list[typing.Any]] | None = None,
    ...
    def assemble_vector(self,
        ...
        _cucpp.fem.assemble_vector_on_device(
            self._ctx, self._cpp_object, b.cuda_form,
    ...
    def assemble_vector_block(self,
        ...
        if len(_constants) != num_forms:
            raise ValueError(
                "Expected constants to be None or a list of length:"
                f" '{num_forms}', got '{len(_constants)}' instead!"
            )
        if len(_coeffs) != num_forms:
            raise ValueError(
                "Expected coeffs to be None or a list of length:"
                f"'{num_forms}', got '{len(_coeffs)}' instead!"
            )
    ...
    def apply_lifting(self,
        b: CUDAVector,
        a: list[CUDAForm],
        bcs: list[typing.Union[list[DirichletBC], CUDADirichletBC]],
        ...
        for bc_collection in bcs:
            if type(bc_collection) is list:
                ...
                raise TypeError(
                    "Expected either a list of DirichletBC's or a CUDADirichletBC,"
