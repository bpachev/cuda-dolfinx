...
int main(int argc, char* argv[]) {
  ...
  const int bs = 2; // each element contains 2 variables (block size)
  ...
  auto element_from = basix::create_element<T>(
      basix::element::family::P, basix::cell::type::triangle, 3,
      basix::element::lagrange_variant::equispaced,
      basix::element::dpc_variant::unset, false);

  auto e0 = std::make_shared<fem::FiniteElement<T>>(element, std::optional<std::vector<std::size_t>> {{bs}});
  auto e1 = std::make_shared<fem::FiniteElement<T>>(element_from, std::optional<std::vector<std::size_t>> {{bs}});

  const auto mesh = std::make_shared<mesh::Mesh<T>>(mesh::create_rectangle(MPI_COMM_WORLD, {{{0., 0.}, {50., 50.}}},
      {100, 100}, mesh::CellType::triangle));
  ...
  f_from->interpolate(
      ...
      {
        std::vector<T> f(bs*x.extent(1)); // 2 x num_points
        for (std::size_t p = 0; p < x.extent(1); ++p)
        {
          f[p] = 1 + 0.1*x(0,p);
          f[x.extent(1) + p] = 1 + 0.2*x(0,p);
        }

        return {f, {bs, x.extent(1)}};
      });
