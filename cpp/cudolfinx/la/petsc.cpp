...
Mat dolfinx::la::petsc::create_cuda_matrix(
    MPI_Comm comm, const dolfinx::la::SparsityPattern &sp) {
  PetscErrorCode ierr;
  Mat A;
  ...
  std::shared_ptr<const dolfinx::common::IndexMap> col_map = sp.index_map(1);
#else
  std::shared_ptr<const dolfinx::common::IndexMap> col_map =
      std::make_shared<const dolfinx::common::IndexMap>(sp.column_index_map());
#endif
  ...
  // The CUDA assembly kernels aren't currently robust to matrices with variable
  // block size So for now always unroll to 1
  _row_ptr.resize(m + 1);
  _row_ptr[0] = 0;
  ...
  for (std::size_t row = 0; row < maps[0]->size_local(); row++) {
    ...
    // row ptr
    for (std::size_t unrolled_row = bs[0] * row;
         unrolled_row < bs[0] * (row + 1); unrolled_row++)
      _row_ptr[unrolled_row + 1] = _row_ptr[unrolled_row] + unrolled_row_nnz;

    for (std::size_t j = 0; j < row_nnz; j++) {
      for (std::size_t k = 0; k < bs[1]; k++)
        _column_indices[unrolled_edge_index + j * bs[1] + k] =
            bs[1] * _edges[edge_index + j] + k;
    }
    // Unroll row block
    for (std::size_t l = 1; l < bs[0]; l++)
      std::copy_n(std::next(_column_indices.begin(), unrolled_edge_index),
                  unrolled_row_nnz,
                  std::next(_column_indices.begin(),
                            unrolled_edge_index + l * unrolled_row_nnz));
  ...
  for (std::size_t i = 0; i < _column_indices.size(); i++) {
    ...
    if (_column_indices[i] < col_local_size)
      ...
      global_column_indices[i] =
          bs[1] * col_ghosts[diff / bs[1]] + diff % bs[1];
    }
  }
  MatCreateMPIAIJWithArrays(comm, m, n, M, N, _row_ptr.data(),
                            global_column_indices.data(), nullptr, &A);
