// Copyright (C) 2024-2026 Benjamin Pachev, James D. Trotter, JørgensS. Dokken
//
...
public:
  ...
  CUDAForm(const CUDA::Context &cuda_context, Form<T, U> *form,
           ufcx_form *ufcx_form,
      ...
      : _coefficients(cuda_context, form, _dofmap_store),
        _constants(cuda_context, form), _form(form), _ufcx_form(ufcx_form),
        _compiled(false) {
    _coefficients =
        CUDAFormCoefficients<T, U>(cuda_context, form, _dofmap_store);
    const int *integral_offsets = ufcx_form->form_integral_offsets;
    if (integral_offsets[3] != integral_tensor_indices.size()) {
      throw std::runtime_error(
          "UFCx form has " + std::to_string(integral_offsets[3]) +
          " integrals, but only " +
          std::to_string(tabulate_tensor_names.size()) +
          " tabulate tensor sources provided to CUDAForm!");
    }
    for (int i = 0; i < 3; i++) {
      for (int offset = integral_offsets[i]; offset < integral_offsets[i + 1];
           offset++) {
        int idx = offset - integral_offsets[i];
        int tensor_offset = integral_tensor_indices[offset];
        _cuda_kernel_sources[i].insert(
            {idx,
             {tabulate_tensor_names[tensor_offset],
              tabulate_tensor_sources[tensor_offset]}});
      }
    }
    ...
    for (IntegralType integral_type :
         {IntegralType::cell, IntegralType::exterior_facet,
          IntegralType::interior_facet, IntegralType::vertex,
          IntegralType::ridge})

    {
      // TODO: add full mixed-topology support when it becomes more mature in
      // dolfinx
  ...
  void compile(const CUDA::Context &cuda_context, int32_t max_threads_per_block,
               int32_t min_blocks_per_multiprocessor, std::string cachedir,
    ...
    if (mesh.geometry().cmaps().size() > 1)
      throw std::runtime_error(
          "CUDAForm does not yet support meshes with multiple cell types.");
    std::int32_t num_vertices_per_cell =
        mesh::num_cell_vertices(mesh.geometry().cmaps()[0].cell_shape());
#else
    std::int32_t num_vertices_per_cell =
        mesh::num_cell_vertices(mesh.geometry().cmap().cell_shape());
#endif
    ...
    if (_form->rank() > 0) {
      ...
      num_dofs_per_cell0 = dofmap0.element_dof_layout().num_dofs() *
                           dofmap0.element_dof_layout().block_size();
    }
    if (_form->rank() > 1) {
    ...
    for (auto &[integral_type, integrals_for_type] : _integrals) {
      auto sources =
          _cuda_kernel_sources[static_cast<std::size_t>(integral_type)];
      for (int i = 0; i < integrals_for_type.size(); i++) {
        ...
        auto assembly_module = compile_form_integral_kernel(
            cuda_context, cachedir, _form->rank(), integral_type, it->second,
            max_threads_per_block, min_blocks_per_multiprocessor,
            num_vertices_per_cell, num_coordinates_per_vertex,
            num_dofs_per_cell0, num_dofs_per_cell1,
            _form->coefficient_offsets().back(), assembly_kernel_type, false,
            false, name);
  ...
  std::map<IntegralType, std::vector<CUDAFormIntegral<T, U>>> &integrals() {
    if (!_compiled) {
      throw std::runtime_error(
          "Cannot access integrals for uncompiled cuda form!");
    }
    return _integrals;
  }
  ...
  std::shared_ptr<const CUDADofMap> unrestricted_dofmap(size_t i) {
    if (i >= _form->function_spaces().size())
      throw std::runtime_error("Dofmap index out of bounds!");
    return _dofmap_store.get_device_object(
        _form->function_spaces()[i]->dofmap().get());
  }

  std::shared_ptr<const CUDADofMap> dofmap(size_t i) {
    if (!restricted())
      return unrestricted_dofmap(i);
    if (i >= _restricted_dofmaps.size())
      throw std::runtime_error("Dofmap index out of bounds!");
  ...
  CUDADirichletBC<T, U>
  bc(const CUDA::Context &cuda_context, size_t i,
     std::vector<std::shared_ptr<const DirichletBC<T, U>>> bcs) {
    return CUDADirichletBC<T, U>(cuda_context, *_form->function_spaces()[i],
                                 bcs);
  }
  ...
  void set_restriction(
      ...
      std::vector<std::shared_ptr<std::map<std::int32_t, std::int32_t>>>
          restriction) {
    if (restriction.size() != _form->function_spaces().size()) {
      throw std::runtime_error("Number of restrictions must equal arity of "
                               "form (1 for vector, 2 for matrix)!");
    }
    _restriction = restriction;
    if (_restricted_dofmaps.size()) {
      // need to update the restriction
      for (int i = 0; i < _restricted_dofmaps.size(); i++) {
        _restricted_dofmaps[i]->update(offsets[i], ghost_offsets[i],
                                       restriction[i].get());
      }
    } else {
      for (int i = 0; i < restriction.size(); i++) {
        _restricted_dofmaps.push_back(std::make_shared<CUDADofMap>(
            _form->function_spaces()[i]->dofmap().get(), offsets[i],
            ghost_offsets[i], restriction[i].get()));
      }
    }
  }

  const std::vector<std::shared_ptr<std::map<std::int32_t, std::int32_t>>>
  get_restriction() {
    return _restriction;
  }
  ...
  std::shared_ptr<dolfinx::common::IndexMap> restriction_index_map(size_t i) {
    ...
    for (auto const &pair : *_restriction[i])
      restricted_inds.push_back(pair.first);
    auto [sub_imap, inds] = dolfinx::common::create_sub_index_map(
        *_form->function_spaces()[0]->dofmap()->index_map, restricted_inds,
...
private:
  ...
  std::array<std::map<int, std::pair<std::string, std::string>>, 4>
